...
def usmi():
    android_version = f"Android {random.randint(4, 13)}.{random.randint(0, 5)}.{random.randint(0, 5)}"
    manufacturer = random.choice(["Samsung", "Xiaomi", "Infinix", "Oppo"])
    ...
    build_version = f"Build/{random.randint(100000, 999999)}"
    fb_version = f"FBAN/FB4A;FBAV/{random.uniform(100, 999):.1f}.{random.uniform(0, 99):.2f}.{random.uniform(0, 99):.2f}"
    fb_locale = random.choice(["en_US", "en_GB", "en_PK", "es_ES", "fr_FR", "de_DE", "pk_PK"])
    fb_brand = manufacturer
    fb_model = manufacturer
    fb_device = random.choice([f"SM-{random.randint(100, 999)}" for _ in range(5)])
    fb_sdk_version = f"FBSV/{random.randint(4, 13)}.{random.randint(0, 5)}.{random.randint(1, 5)}"
    fb_carrier = random.choice(["Zong", "null", "Marshmallow", "Telecom China"])
    fb_cpu = "armeabi-v7a:armeabi"
    fb_screen = f"FBDM/{{density={random.choice(['1.0', '1.5', '2.0', '2.5', '3.0'])},width={random.choice(['720', '1080'])},height={random.choice(['1280', '1440', '1920'])}}}"
    fb_fw = "FB_FW/1"

    user_agent = f"(Dalvik/2.1.0 (Linux; U; {android_version}; {manufacturer} {model} {build_version}); {fb_version}; FBLC/{fb_locale}; FBBN/{fb_brand}; FBMF/{fb_model}; FBDV/{fb_device}; {fb_sdk_version}; FBCA/{fb_cpu}; FBDM/{fb_screen}; {fb_fw})"
